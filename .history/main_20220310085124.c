...
//Global variables
int h, i, ch, k, num = 0, isUnvalid = 1;
int cost_service[5];
FILE *fp, *fp2;
#define home main
...
void add (int h, int i) { 
   ...
   fp2 = fopen("room list 2.txt", "a"); 
	...
	while (isUnvalid  == 1) {
		...
		int month = roomnumber[i].checkin.dd;
		int year = roomnumber[i].checkin.dd;
		printf("%d - %d", day, roomnumber[i].checkin.dd);
		printf("%d - %d", month, roomnumber[i].checkin.mm);
		printf("%d - %d", year, roomnumber[i].checkin.yy);
	...
	while (roomnumber[i].numclient <= cost_room[h].max_people) {
	    ...
	    printf("\n\nEnter your fullname: ");
        scanf("%s", roomnumber[i].client.name);
        printf("\nBirthday (dd/mm/yyyy):");
...
//	fclose(fp);	  
//    num = num + 1; // bieu thi so phong da dc ghi nhan
    
    roomnumber[i].status= 0; // chuyen status tu 1 ve 0
