...
void roomstatus(struct room_hotel *roomnumber){
	...
	for (i = 1; i <= 5; ++i){
		...
		printf("\t %d  \t%s        %d                  %s  \t%d/%d/%d  \t%s\n", i,  cost_room[0].type, cost_room[0].price_per_night, pc, roomnumber[i].checkin.dd, roomnumber[i].checkin.mm, roomnumber[i].checkin.yy, roomnumber[i].checkout );
	}
	for (i = 6; i <= 10; ++i){
		pc = roomnumber[i].status == 1 ? "NO" : "YES";	
		printf("\t %d  \t%s        %d                  %s  \t%d/%d/%d  \t%s\n", i,  cost_room[1].type, cost_room[1].price_per_night, pc, roomnumber[i].checkin.dd, roomnumber[i].checkin.mm, roomnumber[i].checkin.yy, roomnumber[i].checkout);
	}
	for (i = 11; i <= 15; ++i){
		pc = roomnumber[i].status == 1 ? "NO" : "YES";	
		printf("\t %d  \t%s        %d                  %s  \t%d/%d/%d  \t%s\n", i,  cost_room[2].type, cost_room[2].price_per_night, pc, roomnumber[i].checkin.dd, roomnumber[i].checkin.mm, roomnumber[i].checkin.yy, roomnumber[i].checkout );
	}
	for (i = 16; i <= 20; ++i) {
		pc = roomnumber[i].status == 1 ? "NO" : "YES";	
		printf("\t %d  \t%s        %d                 %s  \t%d/%d/%d  \t%s\n", i,  cost_room[3].type, cost_room[3].price_per_night, pc, roomnumber[i].checkin.dd, roomnumber[i].checkin.mm, roomnumber[i].checkin.yy, roomnumber[i].checkout );
	}
}
