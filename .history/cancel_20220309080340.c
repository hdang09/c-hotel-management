...
//  Input a room to cancel
    ...
    while (fgets(line, lineLength, f)) {
		...
		token = strtok(NULL, "/");
		s.birth.dd = strtol(token, NULL, 10);
		token = strtok(NULL, "/");
		s.birth.mm = strtol(token, NULL, 10);
		token = strtok(NULL, ", ");
		s.birth.yy = strtol(token, NULL, 10);

		printf("Token: %s", token);
		printf("\nCheckin mm: %d", s.checkin2.yy);

		strcpy(s.checkin2.mm, strtok(NULL, "/"));
		strcpy(s.checkin2.yy, strtok(NULL, ", "));
		strcpy(s.checkin2.dd, strtok(NULL, "/"));
		strcpy(s.checkin2.mm, strtok(NULL, "/"));
		strcpy(s.checkin2.yy, strtok(NULL, ", "));
//		s.name2 = strtok(NULL, ", ");
