void edit()
{
    ...
    while (fgets(line, sizeof(line), f)) {
		...
		if (atoi(roomNumber) == s.room2) {
			...
			// s.total_pay2 = s.longstay2 * (cost_room[h].price_per_night + s.service_in_use2[1] + s.service_in_use2[2] + s.service_in_use2[3] + s.service_in_use2[4] + s.service_in_use2[5] );
            fprintf(t, "%d\n", s.total_pay2);
            // break;
		} else {
			// Print to "temp.txt"
