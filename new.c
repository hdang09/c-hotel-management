#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <conio.h>

//struct for date 
typedef struct {
	int dd;
	int mm;
	int yy;
} date;

//customer properties
typedef struct {
	char name[50];
	date birth;
	char sex[20];
	int phone[15];
	char email[100];	
} client_detail;

//room type 
struct catetory_room{
 	int max_people;
 	char type[50];
 	int price_per_night;	
} cost_room[4] = {  
	{2, "ROOM TYPE 1", 500000},
   	{5, "ROOM TYPE 2", 700000},
   	{8, "ROOM TYPE 3", 900000},
   	{11,"ROOM TYPE 4", 1100000}
};  

//services 
typedef struct{
	int id;
	char name_service[50];
	int price_service;
} service;

//hotel services cost
service list_service[5] = {
	{1, "service 1", 100000},
	{2, "service 2", 100000},
	{3, "service 3", 200000},
	{4, "service 4", 200000},
	{5, "service 5", 400000}
};

//Struct for a one room 
struct room_hotel {
    int id_room;
 	int numclient;  
 	client_detail client[150];
 	int longstay;
 	date checkin;
 	date checkout; 
 	int service_in_use[5];
 	int status  ; 
 	int total_pay; 	 	
  }roomnumber[21]; // The number of a room is 20 room

//Declare PROTOTYPES
void roomstatus(struct room_hotel *roomnumber);
void add(int h, int i);
int pay(struct room_hotel *roomnumber, int h);
void viewservice();
void home();
void view();
void viewoneroom();
void deleteroom();
void cleanschedule();
//Global variables
int h, i, ch, k, num = 0;
int cost_service[5];
FILE *fp, *S;
#define bookroom main



void bookroom(){
    //SEE EVAILABLE ROOM
    for ( i = 1; i <= 20; ++i){
		roomnumber[i].status = 1; 
	};
	roomstatus(roomnumber);
	  
    //Pick room
    RUN1: printf("\n\n    Which room number you choose: ");
   	scanf("%d", &i);
    	
   	//Check status of this room
	if(roomnumber[i].status == 0 || i > 20 || i < 1) {
		printf("The room you choose is not available now !\n");
		printf("\nPlease pick another room >>   \n\n");
		printf("Return to home press --> \"H\" \n Pick another room press --> \"A\" \n >> ");
	    ch = getch();
		if(ch == 'h' || ch == 'H'){
//			system("cls"); // clear screan
			home();// write code to return to home of hotel 		
		}
    }
	if(ch == 'a' || ch == 'A') goto RUN1;
				
//======================================================================================
		
	//room type 1 max pp is 2
	if( i>= 1 && i <= 5 ){		
		int h= 0; //h bieu thi chi so index cua cau truc kieu mang cost_room[4]
		add(h, i); 
  	}
    
   	//room type 2 max pp is 5
   	if( i>= 6 && i <= 10 ){
		int h= 1;
		add(h, i);
   	}
   	//room type 3 max pp is 8
   	if( i>= 7 && i <= 15 ){
		int h= 2;
		add(h, i);
   	}
   	//room type 4 max pp is 11
   	if( i >= 16 && i <= 20){
   		int h= 3;
   		add(h, i);
	}
}

void add(int h, int i){ 
   //Khai bao con tro file
   
   fp=fopen("room list.txt", "a+"); 
	if(fp==0){
    	fp=fopen("room list.txt", "w");
    //	system("cls");
	    printf("Please hold on while we set our database in your computer !!");
	    printf("\n Process completed press any key to continue !!");
	    getch();
	} 	
	if((S = fopen("service list", "w")) == NULL){
		 printf("Error! opening file");

       // Program exits if the file pointer returns NULL.
       exit(1);	
	}
	//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

	roomnumber[i].numclient = 0;
	roomnumber[i].id_room = i;
    printf("---ROOM %d---\n", i);
    printf("\nPrice per night is %d", cost_room[h].price_per_night);
	printf("\nNote: Max people in %s is %d", cost_room[h].type, cost_room[h].max_people );		   
	printf("\nEnter period(\'x\'days):  ");
    scanf("%d", &roomnumber[i].longstay);
    printf("\nEnter day you checkin (dd/mm/yy): ");
    scanf("%d/%d/%d", &roomnumber[i].checkin.dd, &roomnumber[i].checkin.mm, &roomnumber[i].checkin.yy);
    printf("\nEnter day you checkout (dd/mm/yy): " );
    scanf("%d/%d/%d", &roomnumber[i].checkout.dd, &roomnumber[i].checkout.mm, &roomnumber[i].checkout.yy );	  
    printf("\n--------------------------------------------------");
    
//Ghi file thông tin dat phong    
fprintf(fp,"\n** ROOM %d ** \n Period days %d\n Checkin %d/%d/%d \n checkout %d/%d/%d \n ",
	roomnumber[i].id_room,
	roomnumber[i].longstay,
	roomnumber[i].checkin.dd, roomnumber[i].checkin.mm, roomnumber[i].checkin.yy,
	roomnumber[i].checkout.dd, roomnumber[i].checkout.mm, roomnumber[i].checkout.yy );
	
	printf("\n\n!!---Please fill this form---!!\n");
	
	roomnumber[i].numclient = 1;
	int p=1;
	while(roomnumber[i].numclient <= cost_room[h].max_people){
	   		
		printf("\n>>----Person %d----<<\n", roomnumber[i].numclient);
	    printf("\n\nEnter your fullname: ");
        scanf("%s", roomnumber[i].client[p].name);
        printf("\nBirthday (dd/mm/yyyy):");
        scanf("%d/%d/%d", &roomnumber[i].client[p].birth.dd, &roomnumber[i].client[p].birth.mm, &roomnumber[i].client[p].birth.yy);
        printf("\nSex (M: Male/ F: Famale/ O: Others):  ");
        scanf("%s", roomnumber[i].client[p].sex);
        printf("\nPhone number:   ");
        scanf("%s", roomnumber[i].client[p].phone);
        printf("\nEmail:  ");
        scanf("%s", roomnumber[i].client[p].email); 
        
        
//Ghi file khach hang trong phong				 
fprintf(fp,"\nPerson %d \n>>--------<<\n fullname: %s \n Birthday(mm/dd/yyyy): %d/%d/%d \n Sex: %s \n Phone number: %s \n Email: %s", 
	    roomnumber[i].numclient, roomnumber[i].client[p].name,
	    roomnumber[i].client[p].birth.dd, roomnumber[i].client[p].birth.mm, roomnumber[i].client[p].birth.yy,
	    roomnumber[i].client[p].sex,
	    roomnumber[i].client[p].phone,
	    roomnumber[i].client[p].email);   
	    
        printf("\n\nDo you want to add one more people press \"Y\" \n If NO press \"N\"");      	     		
        printf("\nY/N >>>  ");
        ch= getch();
        if(ch == 'N' || ch == 'n')
		  	break; 
       	if(ch == 'Y' || ch == 'y') {
         	roomnumber[i].numclient ++;
         	p++;
     	    if( roomnumber[i].numclient > cost_room[h].max_people) {
     	       	printf("Sorry the room has max people !\n");
     	        break;   					
    	   }
    	}
   } 
    printf("---------------------------------------------");
     		
	//Book service
    printf("\n\nWe have these services ");
    printf("\n1. Service 1 cost 100000\n");
    printf("2. Service 2 cost 100000\n ");
    printf("3. Service 3 cost 200000\n");
    printf("4. Service 4 cost 200000\n");
    printf("5. Service 5 cost 400000\n");
    printf("\nNote: if you choose service 1 and 2 just type \" 1_1_0_0_0 \" with \"_\" mean space, \"0\" mean you don't want \n");
    printf("Your option: ");
    int a = 1;
    while (a <= 5){
       	scanf("%d", &roomnumber[i].service_in_use[a]);
       	a++;
       	
    }		
	//calculate totalpay
	roomnumber[i].total_pay = pay(roomnumber, h);	
    printf("\nYour total bill: %d VND ", roomnumber[i].total_pay ); 
fprintf(fp,"\n\n Total pay: %d", roomnumber[i].total_pay);

	fclose(fp);	  
	
    //ghi nhan status room luc nay = 0 --> có khách
    roomnumber[i].status= 0; 
    printf("Succesfully requested booking\n");
    
  
	printf("\nPress B --> Return to home");
    ch=getch();
    if(ch == 'B' || ch == 'b'){
//    	system("cls");
    	home();
		 //code return to home
	
	}	          	
}

//Pay function

int pay(struct room_hotel *roomnumber, int h){
	int payy;
	int a = 1;

		
fprintf(fp,"\n\nServices in use \n Service 1 \t Service 2 \t Service 3 \t Service 4 \t Service 5 \n ");
fprintf(S,"%d     ", roomnumber[i].id_room);//ghi so phong xai dich vu
	while (a <= 5){    	    			
		if(roomnumber[i].service_in_use[a] == 1 ){
			       cost_service[a] = list_service[a - 1].price_service;
		    fprintf(fp, "YES                ");
		    fprintf(S, "YES                ");
		}
		else{
			       cost_service[a]= 0;
			fprintf(fp, "NO                ");
			fprintf(S, "NO                ");		
			    }
		a++;    					
	}
	payy= roomnumber[i].longstay * (cost_room[h].price_per_night + 
		                            cost_service[1] + cost_service[2] + 
									cost_service[3] + cost_service[4] + cost_service[5] );
			                                                
	return (payy);
}

void roomstatus(struct room_hotel *roomnumber){
	char *pc;
		
    printf("\n\tRoom   \t  Category  \tPrice per night  \tAvailable \n");				
	for (i=01; i<=05; ++i){
		    if(roomnumber[i].status == 0) pc="NO";
	        else pc="YES";
		printf("\t %d  \t%s        %d                  %s\n", i,  cost_room[0].type, cost_room[0].price_per_night, pc);
	}
	for (i=06; i<=10; ++i){
		    if(roomnumber[i].status == 0) pc="NO";
	        else pc="YES";
		printf("\t %d  \t%s        %d                  %s\n", i,  cost_room[1].type, cost_room[1].price_per_night, pc);
	}
	for (i=11; i<=15; ++i){
		    if(roomnumber[i].status == 0) pc="NO";
	        else pc="YES";
		printf("\t %d  \t%s        %d                  %s\n", i,  cost_room[2].type, cost_room[2].price_per_night, pc);
	}
	for (i=16; i<=20; ++i){
		    if(roomnumber[i].status == 0) pc="NO";
	        else pc="YES";
		printf("\t %d  \t%s        %d                 %s\n", i,  cost_room[3].type, cost_room[3].price_per_night, pc );
	}
}

void home(){
	printf("hihihi");
	
}

void view(){
	fp = NULL;
	char arr[100000];
	fp = fopen("room list.txt", "r");
	while(fgets(arr, 100000, fp) != NULL)
	{
		printf("%s", arr);
	}
	fclose(fp);
}

void viewoneroom(){
	int k, p = 1, a=1;
	RUN1: printf(" Choose room you want to view:  ");
	scanf("%d", &k);
	if(k < 1 || k > 20) goto RUN1;
	for(i=1; i <= 20; i++){
		if(roomnumber[i].id_room == k){
			printf(" \nRoom %d\n", roomnumber[i].id_room);
			printf(" Checkin %d/%d/%d\n", roomnumber[i].checkin.dd, roomnumber[i].checkin.mm, roomnumber[i].checkin.yy);
			printf(" Checkout %d/%d/%d\n", roomnumber[i].checkout.dd, roomnumber[i].checkout.mm, roomnumber[i].checkout.yy);
			while(p <= roomnumber[i].numclient){
				printf(" \nPerson %d\n", p);
				printf(" Fullname: %s \n", roomnumber[i].client[p].name);
				printf(" Birthday: %d/%d/%d \n", roomnumber[i].client[p].birth.dd, roomnumber[i].client[p].birth.mm, roomnumber[i].client[p].birth.yy);
				printf(" Sex: %s \n", roomnumber[i].client[p].sex);
				printf(" Phone number: %s \n", roomnumber[i].client[p].phone);
				printf(" Email: %s\n", roomnumber[i].client[p].email );
				p++;
			}
			printf("\n\nServices in use \n Service 1 \t Service 2 \t Service 3 \t Service 4 \t Service 5 \n ");
			while(a <= 5){
				if(roomnumber[i].service_in_use[a] == 1)
				printf("YES                ");
				else
				printf("NO                ");
				a++;
			}
			printf("\n\n TOTALPAY: %d", roomnumber[i].total_pay);
			break;	
	    }
	    printf("\nCan't found anything !\n");
	}   		
}

void deleteroom(){
	int k,i, size = 21;
	RUN2: printf("Enter the room : ");
	scanf("%d", &k);
	
	if(k < 1 ||  k > 20){
		printf("Invalid number! Please enter the room from 1 to 20 ");
		goto RUN2;
	}
	else{
		roomnumber[k].status = 1;
		for(i= k; i <= size - 1; i++){
			roomnumber[i] = roomnumber[i+1];
			size--;	
		}
	}	
}

void viewservice(){
	printf("\n\n Room    Service 1 \t Service 2 \t Service 3 \t Service 4 \t Service 5 \n ");
	for(i=1; i<=20; i++){
		if(roomnumber[i].status == 0){
			printf("%d     ", i);
			int a=1;
			while(a<=5){
				if(roomnumber[i].service_in_use[a] == 1)
			    printf( "  YES                ");
			    else
		        printf( "NO                ");	
				a++;		
			}
		}
	}
}

void cleanschedule(){
	
	FILE *clean ;
	if((clean = fopen("Clean room.txt", "a+")) == NULL){
		printf("ERROR\n");
		exit(1);
	}
	
	clean = NULL;
	char h[50];
	clean = fopen("Clean room.txt", "a+");
	while(fgets(h, 50, clean) != NULL)
	{
		printf("%s", h);
	}
	fclose(fp);
}